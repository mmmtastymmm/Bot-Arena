...
impl Player {
    ...
    pub fn deal(&mut self, cards: [Card; 2]) {
        self.player_state = PlayerState::Active { 0: ActiveState { hand: Hand { cards }, current_bet: Bet { bet: 0 } } };
    }
}
...
impl Table {
    ...
    pub fn deal(&mut self) {
        ...
        self.flop = Option::from([deck_iterator.next().unwrap().clone(),
            deck_iterator.next().unwrap().clone(),
            deck_iterator.next().unwrap().clone(),
        ]);

        self.turn = Option::from(deck_iterator.next().unwrap().clone());
        self.river = Option::from(deck_iterator.next().unwrap().clone());

        for player in &mut self.players {
            let card1 = deck_iterator.next().unwrap().clone();
            let card2 = deck_iterator.next().unwrap().clone();
